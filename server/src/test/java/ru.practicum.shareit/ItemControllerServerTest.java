...
public class ItemControllerServerTest {
    ...
    private static String getJson(String name, Object... args) throws IOException {
        try (InputStream resourceAsStream = requireNonNull(ItemControllerServerTest.class.getResourceAsStream(name))) {
    ...
    void whenSearchWithoutText_thenStatus200andEmptyListReturned() throws Exception {
        ...
        mockMvc.perform(get("/items/search?text=&from=0&size=15")
                        .characterEncoding(StandardCharsets.UTF_8)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON)
                )
